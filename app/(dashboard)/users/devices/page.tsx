...
export default async function DevicesPage() {
  ...
  const columns: Column<PushDevice>[] = [
    ...
    { header: "Push", cell: (d) => <Badge tone={d.enabled ? "green" : "neutral"}>{d.enabled ? "An" : "Aus"}</Badge> },
    { header: "News-Push", cell: (d) => <Badge tone={d.news_enabled ? "green" : "neutral"}>{d.news_enabled ? "An" : "Aus"}</Badge> },
    { header: "Favoriten", cell: (d) => fmt(d.favorite_count) },
    { header: "Tickets", cell: (d) => fmt(d.ticket_count) },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-neutral-900">Geräte</h1>
